...
export class AccountMyShoppingComponent implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		// 	if(Object.keys(resp)[0] == "-MMbwSSkVb6SHZwIEyqp"){
		// 		this.is_admin = true;
		// 	}
		// })
	  	/*=============================================
					...
					.subscribe(resp=>{
						if(Object.keys(resp).length > 0){
							...
							for(const i in resp){
								...
								.subscribe(resp1=>{
									this.id_user = Object.keys(resp1)[0];
									for(const f in resp1){
										resp[i].user = resp1[f];
									}
	...
	// Formulario de disputas
	onSubmit(f: NgForm){

		// Validamos el formulario para evitar campos vacios
		if(f.invalid){
			Sweetalert.fnc("error", "Invalid Request", null);
			return;
		}
		// Creamos una disputa en la base de datos
	...
	newReview(user, url){
		...
		/*=============================================
		Traemos el Usuario para capturar Imagen
		=============================================*/
	...
	submitReview(comment, review){
		...
		.subscribe(resp=>{
			...
			// Eliminar la reseña creada anteriormente por el usuario
			...
			.subscribe(resp=>{
				if(resp["reviews"] != ""){
					Sweetalert.fnc("success", "The review was created seccessfully", "account/my-shopping");
				}
			}, err =>{
				Sweetalert.fnc("error", err.error.error.message, null);
			})
