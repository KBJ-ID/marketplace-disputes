  ...
  onSubmit(f:NgForm){
    ...
    .subscribe(resp=>{
      if(resp["name"] != ""){
        ...
        // Enviar notificación por correo electronico
        const formData = new FormData();
        formData.append('email', 'yes');
        formData.append('comment', 'You have received an answer on your question');
        formData.append('url', 'product/'+this.message.url_product);
        ...
        .subscribe(resp=>{
          if(resp["status"] == 200){
            Sweetalert.fnc("success", "The dispute has been answered", "account/messages");   
          }
        })
      }
