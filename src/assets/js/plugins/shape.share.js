  ...
  $.fn.shapeShare = function (options) {
    ...
    function getData(object) {
      for (var index = 0; index < dataObjects.length; index++) {
        dataVals[dataObjects[index]] = $(
          'meta[property="og:' + dataObjects[index] + '"]'
        ).attr("content");
      }
    ...
    function contentTwitter() {
      var contentCalculator =
          dataLengths.url +
          dataLengths.hashtags +
          (dataLengths.account + contentTwitterAdjust),
        content =
          contentSlice(settings.twitter.content, contentCalculator) +
          " via @" +
          settings.twitter.account +
          " " +
          settings.hashtags,
      ...
      switch (shareType) {
        case "facebook":
          shareURL =
            ...
            encodeURIComponent(dataVals.url);
          break;
        ...
        case "twitter":
          shareURL =
            ...
            encodeURIComponent(dataVals.url) +
            "&text=" +
            encodeURIComponent(contentTwitter()) +
            "&url=" +
            encodeURIComponent(dataVals.url);
          break;
        ...
        case "pinterest":
          shareURL =
            ...
            encodeURIComponent(dataVals.url) +
            "&media=" +
            encodeURIComponent(dataVals.image) +
            "&description=" +
            encodeURIComponent(dataVals.description);
          break;
      ...
      switch (settings.window) {
        case "popup":
          window.open(
            shareURL,
            shareType,
            "menubar=no,toolbar=0,status=0,width=" +
              settings.popupSize.width +
              ",height=" +
              settings.popupSize.height
          );
          break;
